...
use std::{cmp::max, marker::PhantomData, sync::Mutex};
...
pub struct ChopinTiming {
  /// time spent committing to pi1, pi2 computed in rayon join
  pub commit_pi_parallel_ms: f64,
  /// time spent committing pi1 (milliseconds)
  pub commit_pi1_ms: f64,
  /// time spent committing pi2 (milliseconds)
  pub commit_pi2_ms: f64,
}

/// global timing measurements
static CHOPIN_TIMINGS: Mutex<ChopinTiming> = Mutex::new(ChopinTiming {
  commit_pi_parallel_ms: 0.0,
  commit_pi1_ms: 0.0,
  commit_pi2_ms: 0.0,
});
...
impl ChopinTiming {
  ...
  /// set timing
  pub fn set_commit_pi_parallel(ms: f64) {
    CHOPIN_TIMINGS.lock().unwrap().commit_pi_parallel_ms = ms;
  }

  /// set timing
  pub fn set_commit_pi1(ms: f64) {
    CHOPIN_TIMINGS.lock().unwrap().commit_pi1_ms = ms;
  }

  /// set timing
  pub fn set_commit_pi2(ms: f64) {
    CHOPIN_TIMINGS.lock().unwrap().commit_pi2_ms = ms;
  }
}
...
type ProverKey<E> = bivariatekzg::ProverKey<E>;
type VerifierKey<E> = bivariatekzg::VerifierKey<E>;
type Commitment<E> = bivariatekzg::Commitment<E>;
type CommitmentEngine<E> = bivariatekzg::CommitmentEngine<E>;
...
impl<Scalar: PrimeField> UniPoly<Scalar> {
  fn expand_chopin(&mut self, size: usize) {
  ...
  pub fn scale_chopin(&mut self, s: &Scalar) {
  ...
  fn from_evals_with_xs_chopin(xs: &[&Scalar], evals: &[Scalar]) -> Self {
    if evals.len() == 1 {
      return Self {
        coeffs: vec![evals[0]],
      };
  ...
  // f(X) * (X + a)
  fn multiply_by_linear_polynomial_chopin(&mut self, a: &Scalar) {
  ...
  // Horner's method
  fn divide_by_linear_polynomial_chopin(&mut self, a: &Scalar) -> Scalar {
  ...
  fn transpose_chopin(&mut self, num_rows: usize, num_cols: usize) {
    assert!(num_rows <= num_cols);

    let b = num_cols;
    self.expand_chopin(b * b);
...
fn divide_by_binomial<Scalar: PrimeField>(
  ...
  let (quotients, remainder): (Vec<Vec<Scalar>>, Vec<Scalar>) = (0..num_cols)
    ...
    .map(|col_id| {
      ...
      quotient.expand_chopin(num_cols);
  ...
  quotient.transpose_chopin(num_rows, num_cols);
...
mod batch_evaluation {
  ...
  use crate::{
    errors::NovaError,
    provider::{chopin::CommitmentEngine, traits::PairingGroup},
    ...
    E::GE: PairingGroup,
  {
    ...
    let [g_star, h_star, s_star] = {
      ...
      eval_domains
        .into_par_iter()
        .zip(evals)
        .map(|(xs, evals)| UniPoly::from_evals_with_xs_chopin(&xs, &evals))
        .collect::<Vec<_>>()
        .try_into()
        .unwrap()
    };
      ...
      polys
        ...
        .for_each(|(poly, vanishing_points)| {
          for p in vanishing_points {
            poly.multiply_by_linear_polynomial_chopin(&-p);
          }
        });
    ...
    let rem = quot_m_poly.divide_by_linear_polynomial_chopin(&alpha);
    assert_eq!(rem, E::Scalar::ZERO);
    let rem = quot_m_poly.divide_by_linear_polynomial_chopin(&zeta);
    assert_eq!(rem, E::Scalar::ZERO);
    let rem = quot_m_poly.divide_by_linear_polynomial_chopin(&zeta_inv);
    assert_eq!(rem, E::Scalar::ZERO);
    ...
    let l_poly = {
      ...
      // L(X) = mz(x) - zt(z) * quot_m(X)
      {
        ...
        lhs.batch_add_with_polynomials_chopin(vec![&quot_m_poly.coeffs], &[-t_eval_at_z]);
    ...
    let quot_l_poly = {
      ...
      let rem = quot_l_poly.divide_by_linear_polynomial_chopin(&z);
      assert_eq!(rem, E::Scalar::ZERO);
    ...
    E::GE: PairingGroup,
  {
    ...
    let g_star =
      UniPoly::from_evals_with_xs_chopin(&[&zeta, &zeta_inv], &[evals.g_zeta, evals.g_zeta_inv]);
    let h_star = UniPoly::from_evals_with_xs_chopin(
      &[&zeta, &zeta_inv, &alpha],
  ...
  E::GE: PairingGroup,
{
  ...
  fn setup(
    ...
    bivariatekzg::EvaluationEngine::setup(ck)
  }
